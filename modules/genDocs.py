    ...
    if files:
        ...
        if not trigger and st.session_state.inserted_file:
            ...
            if len(final_mp) and success:
                ...
                mapped_target_cols = set(final_mp.values())
                unmapped_cols = [col for col in df_out.columns if col not in mapped_target_cols]
                molded_df = clean_dataframe_types(df_out, only_cols=unmapped_cols)
        ...
        # 1. Create a container for filters at the top
        with st.popover("🔍 Filter Table Options", use_container_width=True):
            filtered_df = molded_df.copy()
            
            # Create columns inside the popover to make filters side-by-side (optional)
            cols = st.columns(3) 
            ...
            for i, column in enumerate(molded_df.columns):
                ...
                # Distribute filters across the internal columns
                with cols[i % 3]:
